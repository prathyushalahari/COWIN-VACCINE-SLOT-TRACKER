...
# telegram token to get slot details
...
def botpincode(update, context):
    ...
    if(len(context.args) == 3):
        ...
        update.message.reply_text("Invalid Example- /botpincode 507002 18 1")
        return
                ...
                if len(json_data["sessions"]):
                    for session in json_data["sessions"]:
                        if session[dose] > 0 and session["min_age_limit"] <= age:
                            ...
                            return
        update.message.reply_text("We'll notify further when any Slots Available!\n")
        sleep(TIME_GAP)
                ...
                if len(json_data["sessions"]):
                    for session in json_data["sessions"]:
                        if session[dose] > 0 and session["min_age_limit"] <= age:
                            msg = f"{session['vaccine']} available for {session[dose]} people  for Dose {dosec}  on {session['date']} at {session['name']},{session['address']}\n"
                            update.message.reply_text(
                                msg, "Link to Book Slots : https://www.cowin.gov.in/")
                            return
        update.message.reply_text("We'll notify further when any slots Available!\n")
        sleep(TIME_GAP)
...
def help(update, context):
    ...
    /botdistrict district age dose\n
    Example: /botdistrict khammam 18 2\n
